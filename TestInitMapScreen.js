...
import {LINHAS, TerminalChegada, NomeLinhas} from './src/itinerario';
...
export class TestInitMapScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      durationBus_Pnt: 0,
  ...
  GetPoints(destination, location) {
    let PointIn = null;
    let PointOut = null;
    let WayPointsAS = []; //AfterStation
    let WayPointsBS = []; //BeforeStation
    //================================
    let StopdistB = -1;
    let StopNumB = -1;
    let StopIndexB = -1;
    ...
    if (distDest < 0.003) {
      Alert.alert(
        'Alerta',
        'Você está a menos de 300 metros do destino selecionado',
        [
          {
            text: 'Ok',
            onPress: () => {},
      ...
      <SafeAreaView
        ...
        >
          <Text
            onPress={() => {
              ...
              if (
                channel.slice(2, 6) === LINHAS[this.state.PointIn.id][0].linha
              ) {
                //console.log('Channel Linha 1 =======================');
                if (
                  this.state.Point_Now <= LINHAS[this.state.PointIn.id].length
                ) {
                ...
                let message =
                  ...
                  ', "Location":' +
                  '{"latitude":' +
                  LINHAS[this.state.PointIn.id][Point_Now].latitude +
                  ',"longitude":' +
                  LINHAS[this.state.PointIn.id][Point_Now].longitude +
                  '}, "RequestOk" : [2, 3, 10] }';
              ...
              if (
                channel.slice(2, 6) === LINHAS[this.state.PointOut.id][0].linha
              ) {
                //console.log('Channe Linha 2 =======================');
                if (
                  this.state.Point_Now <= LINHAS[this.state.PointOut.id].length
                ) {
                  Point_Now = this.state.Point_Now + 1;
                }
                let message =
                  ...
                  ', "Location":' +
                  '{"latitude":' +
                  LINHAS[this.state.PointOut.id][Point_Now].latitude +
                  ',"longitude":' +
                  LINHAS[this.state.PointOut.id][Point_Now].longitude +
                  '}, "RequestOk" : [] }';
        ...
        <View
          ...
          <MapView
            ...
            {this.state.WayPointsAS.map((WayPointsAS, index) => (
              <MapView.Marker
                coordinate={{
                  latitude: WayPointsAS.latitude,
                  longitude: WayPointsAS.longitude,
                }}
                key={index}
                image={IMAGE.STOP}
              />
            ))}
            {this.state.WayPointsBS.map((WayPointsBS, index) => (
              <MapView.Marker
                coordinate={{
                  latitude: WayPointsBS.latitude,
                  longitude: WayPointsBS.longitude,
                }}
                key={index}
                image={IMAGE.STOP}
              />
